...
do
    ...
    UILibrary:CreateButton({Name = "edgebug", Tab = "Misc", Section = "Movement", KeyBind = "None"})
    UILibrary:CreateSlider({Name = "edgebug Speed", Tab = "Misc", Section = "Movement", MinimumNumber = 0, MaximumNumber = 400, Suffix = " st/sec"})
	...
	UILibrary:CreateButton({Name = "Realistic JP", Tab = "Misc", Section = "Tweaks"})
...
do --ANCHOR Movement
    ...
    function movement.step(delta)
        ...
        local flightmode = (Menu["Misc"]["Movement"]["edgebug"]["Toggle"]["Enabled"] and Menu["Misc"]["Movement"]["edgebug"]["Bind"]["Active"])
        if localCharacter then
            ...
            if humanoid and humanoidRootPart then
                ...
                local cfRel = flightmode and camera.CFrame or CFrame.lookAt(camera.CFrame.p, camera.CFrame.p + camera.CFrame.LookVector)
                if userInputService:IsKeyDown("W") then
                    --moveDirection = moveDirection + cfRel.LookVector
                    moveDirection = Vector3.new(cfRel.LookVector.X, 0, cfRel.LookVector.Z).Unit
                end
                ...
                if (Menu["Misc"]["Movement"]["edgebug"]["Toggle"]["Enabled"] and Menu["Misc"]["Movement"]["edgebug"]["Bind"]["Active"]) then
                    local speed = Menu["Misc"]["Movement"]["edgebug Speed"]["Value"]
                ...
                if Menu["Misc"]["Tweaks"]["Realistic JP"]["Toggle"]["Enabled"] then
...
--тут начинается пиздец не смотрите сюда(да мне писал скрипты чат гпт хд)
...
local function sendChatMessage(senderName, messageText, senderColor, messageColor, displayDuration)
    local chatModule = getsenv(game.Players.LocalPlayer.PlayerGui.GUI.Main.Chats.DisplayChat)

    if chatModule then
        chatModule.moveOldMessages() -- Очищаем старые сообщения
        chatModule.createNewMessage(senderName, messageText, senderColor, messageColor, displayDuration)
    else
        warn("Chat module not found")
    end
end

-- Обработчик нажатия клавиш
userInputService.InputBegan:Connect(function(input, isProcessed)
    -- Проверяем, что клавиша Alt нажата, и это событие не обработано другим скриптом
    if input.KeyCode == Enum.KeyCode.LeftAlt and not isProcessed then
        sendChatMessage(
            "arterware.shit", 
            "edgebugged", 
...
local function playSound()
    -- Проверяем, существует ли звук в Workspace
    local sound = workspace:FindFirstChild("AltSound")
    if not sound then
        sound = Instance.new("Sound")
        sound.Name = "AltSound"
        sound.SoundId = "rbxassetid://6887181639" -- ID звука
        sound.Volume = 1 -- Уровень громкости
        sound.Looped = false -- Звук не будет повторяться
        sound.Parent = workspace -- Размещаем звук в Workspace
    end
